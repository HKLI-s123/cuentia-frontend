...
import { getMenuForAccount } from "@/utils/getMenuByTipoCuenta";
...
const AppMenu = () => {
    ...
    // 👇 Obtener tipoCuenta desde el backend
    useEffect(() => {
      const load = async () => {
        try {
          ...
          setTipoCuenta("individual"); // fallback
        }
      };
    ...
    // Mientras carga
    const isLoading = tipoCuenta === null;

    const filteredMenu = getMenuForAccount(tipoCuenta);
    ...
    return (
     <ul className="side-nav">
        {isLoading ? (
          <li className="side-nav-title">Cargando menú...</li>
        ) : (
